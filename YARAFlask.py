...
def trade_text():
    ...
    DJ_Name_Match = ['mmm', 'aapl', 'mmm', 'mmm', 'vz', 'v', 'wmt', 'wmt', 'wmt', 'wmt', 'trv', 'utx', 'utx', 'unh',
                     ...
                     'wmt', 'trv', 'utx', \
                     'unh', 'msft', 'nke', 'pfe', 'pg', 'axp', 'jpm', 'mcd', 'mrk', 'jnj', 'intc', 'ibm', 'gs', 'hd',
                     'ge', 'xom', 'ba', \
    ...
    buy_trade = ['buuy','Buuy','byu','Byu','Purchase', 'purchase', 'trade', 'invest', 'place', 'buy', 'by', 'long', 'Long', 'Lawn', 'lawn', 'Lon',
                 'Buy']
    sell_trade = ['sell', 'Sell', 'so ',' so', 'So',' So', 'Settle', 'settle', 'sel', 'Sel']
    ...
    shares_key = ["shares", "share", "Shares", "Share"]
        ...
        if any(word in phrase1 for word in sell_trade):
            ...
            if not any(word in tickers[0].upper() for word in tickers_check):
                ...
                if int(shares) > shares_port:
                    ...
                    result = ("You are " + buy_or_sell.lower() + "ing " + str(shares_port - int(shares)) + " shares of " + security.upper() + " at a price of " + price + \
                              " for $" + "{0:,.2f}".format(invest_amt) + " . I will round down on the shares if funds don't meet the order requirements. If this is correct, please confirm order." + '<br />' + '<br />' + \
                              "Security: " + security.upper() + '<br />' +
                              "Buy or Sell: " + buy_or_sell + '<br /  >' +
                              "Order Type: " + order_type_send + '<br />' +
                              "Price: " + price + '<br />' +
                              "Shares: " + str(shares_port - int(shares)) + '<br />' +
                              "Investment Amount: $" + "{0:,.2f}".format(invest_amt))
...
#     print(phrase1)
#
#     backtest = ['bath test','black test','Black Test','Black test','black Test','back test','Back test','Back Test','back Test','backest','Backest','baktest','Baktest','backest','Backest','bcacktest','Bcacktest','backtes','Backtes','bcktest','Bcktest','bakctes','Bakctes','bakctest','Bakctest','bactest','Bactest','backtet','Backtet','backtestnig','Backtestnig','backtset','Backtset','backtest','Backtest','backtesting','Backtesting','bakctest','Bakctest']
#
